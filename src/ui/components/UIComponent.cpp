...
void UIComponent::SetStyle(const std::unordered_map<std::string, std::any>& styleMap) {
    const std::unordered_map<std::string, std::function<void(UIStyle&, const std::any&)>> setters = {
    ...
    for (const auto& it1 : styleMap)
    {
        if (auto it2 = setters.find(it1.first); it2 != setters.end()) {
            it2->second(style, it1.second);
        }
    }
